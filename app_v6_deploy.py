...
def render_usage_guide_for_hunters() -> None:
    st.title("📖 App 使用說明（工程師）")
    st.caption("AI 智慧派工系統｜工程標案 / 維修派單 / 分潤結算｜工程師操作指南")

    with st.expander("0️⃣ 你會在這個系統做什麼？（一句話）", expanded=True):
        st.markdown(
            """
- 看到公司釋出的 **工程標案 / 維修派單**
- 依規則 **接單（或投標）→ 執行 → 完工回報**
- 主管驗收通過後（Done），系統把該筆金額納入你的 **本月貢獻營業額** 與 **排行榜 / 團隊牆**
"""
        )

    with st.expander("1️⃣ 登入與基本介面", expanded=True):
        st.markdown(
            """
**登入：**
- 從「同仁入口」選姓名 → 輸入密碼 → 點「⚡ 上工」
- 登入後左側邊欄會顯示：你的姓名 + 所屬分組（工程1/工程2/維養1）
...
"""
    ...
    with st.expander("2️⃣ 先看 KPI 橫幅：本月目標進度 / 達標狀態", expanded=True):
        st.markdown(
            """
登入後最上方會看到 KPI 橫幅：
- **實拿業績（本月）**：依系統結算規則計算（只統計 `Done` 且 `created_at` 在本月的任務）
- **目標金額**：系統預設 250,000
- **等級徽章**：依金額門檻顯示（起步/進階/菁英/傳奇/神話）
- **達標動畫**：達標只會噴一次（避免一直 rerun 一直噴）
- **Streak**：連續達標會累積；若本月未達標會歸零（以你程式當前規則為準）
"""
        )

    with st.expander("3️⃣ 更新任務（很重要）", expanded=True):
        st.markdown(
            """
畫面上方有「🔄 更新任務」按鈕：
- 系統有快取（cache），不是每秒都會自動刷新
- 若你看到「✅ 已是最新」代表目前畫面資料已是最新
- 若有新任務或資料過期，按鈕會出現並可點擊同步
- 點擊後會清快取並抓最新資料（看到 toast：✅ 已同步最新任務）
"""
        )

    with st.expander("4️⃣ 三個工作頁籤：工程標案 / 維修派單 / 我的任務", expanded=True):
        st.markdown(
            """
你會在上方看到三個 tab：
- **🏗️ 工程標案**：可挑工程案投標、可選隊友（最多 3 人）
- **🔧 維修派單**：快速接單（通常不需要隊友）
- **📂 我的任務**：你目前進行中/待驗收的任務都在這裡
"""
        )

    with st.expander("5️⃣ 🏗️ 工程標案：投標流程（含隊友）", expanded=True):
        st.markdown(
            """
**流程：**
1. 進入「🏗️ 工程標案」
2. 每張卡片會看到：類別、金額、估價單號（若有）、說明
3. 如需隊友：用「🤝 找隊友」選人（最多 3 人）
4. 按「⚡ 投標」  
   - 成功後任務狀態會變 `Active`
   - 你（hunter_id）與隊友（partner_id）會被寫入該任務

**注意：忙碌鎖定**
- 只要你在任何 `Active` 任務（含你是隊友），系統會顯示「🚫 任務進行中」
- 忙碌時：工程投標/維修接單按鈕會被鎖住，避免同時接多單
"""
        )

    with st.expander("6️⃣ 🔧 維修派單：接單流程（快速）", expanded=True):
        st.markdown(
            """
**流程：**
1. 進入「🔧 維修派單」
2. 每張卡片會看到：類別、金額、估價單號（若有）、說明  
   - 若是「緊急搶修」會有 🔥URGENT 標示
3. 按「✋ 我來處理」接單  
   - 成功後狀態變 `Active`，你會進入忙碌鎖定
"""
        )

    with st.expander("7️⃣ 📂 我的任務：完工回報（解除鎖定）", expanded=True):
        st.markdown(
            """
**你接到的任務會出現在「📂 我的任務」：**
- `Active`：進行中（你是主承接 hunter_id 或隊友 partner）
- `Pending`：你已完工回報，等待主管驗收

**完工回報：**
- 只有你是主承接（hunter_id）且狀態 `Active` 時，會看到「📩 完工回報 (解除鎖定)」
- 點下去後狀態會變 `Pending`  
  （此時你通常會解除忙碌鎖定，等待主管驗收）
"""
        )

    with st.expander("8️⃣ 主管驗收通過後（Done）會發生什麼？", expanded=True):
        st.markdown(
            """
當主管在「驗收審核」按「✅ 通過」後，任務狀態變 `Done`：
- 該筆金額會納入本月結算（以任務 `created_at` 所屬月份計）
- 會影響：
  - 你的 KPI 橫幅金額
  - 團隊牆分佈（已達標 / 衝刺中 / 穩定推進 / 起步中）
  - 匿名排行榜（Top1/2/3 區間）
"""
        )

    with st.expander("9️⃣ 分潤結算規則（工程師版簡表）", expanded=True):
        st.markdown(
            """
你的系統目前核心結算邏輯（以程式現況為準）：

**A) 結算範圍**
- 只計算 `status == Done`
- 且 `created_at` 需符合當月 `YYYY-MM`（例如 2026-01）

**B) 任務金額（points / maint_points）**
- 若是維養類別（在 `TYPE_MAINT`）：優先用 `maint_points`，若為 0 才用 `points`
- 若是工程類別（在 `TYPE_ENG`）：一律用 `points`

**C) 分配方式（均分 + 餘數給主承接）**
- 團隊 = hunter_id（主承接）+ partner_id（隊友清單）
- 金額均分：`amount // 人數`
- 餘數：全給主承接（hunter_id）

**D) 特例：維養轉介（80/20）**
- 若任務 `source_type == "維養轉介"`：
  - 工程團隊拿 80%（再依均分+餘數規則）
  - 維養來源人 `source_hunter_id` 拿 20%
"""
        )
    ...
    with st.expander("🔟 常見問題（QA）", expanded=False):
        st.markdown(
            """
**Q1：為什麼我接單後同事都不能接？**  
A：系統有「忙碌鎖定」：你在任何 `Active` 任務（含隊友）就會鎖住接新單，避免同時接多單。

**Q2：我完工了怎麼讓金額進帳？**  
A：你先在「我的任務」按「完工回報」→ 狀態變 `Pending` → 主管驗收通過後變 `Done` 才會計入本月。

**Q3：排行榜為什麼沒有名字？**  
A：工程師端是匿名排行榜（只顯示 Top 區間），避免造成點名壓力；主管端才會看到明細。

**Q4：我看不到最新任務？**  
A：先按「🔄 更新任務」同步（系統有快取，不會每秒刷新）。
"""
        )

    st.divider()
    st.caption("若你發現任務資料不合理（估價單號/金額/類別錯誤），請回報主管由指揮台修正。")
